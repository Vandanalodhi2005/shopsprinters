...
const AdminSidebar = ({ isOpen, setIsOpen, currentPage, setPage, logout }) => {
  const navItems = [
    { name: 'Dashboard', id: 'admin-dashboard', icon: (
    ...
    { name: 'Categories', id: 'admin-categories', icon: (
    ...
    { name: 'Customer Chat', id: 'admin-chat', icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
    ) },
    { name: 'Analytics', id: 'admin-analytics', icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
    ) },
    { name: 'Settings', id: 'admin-settings', icon: (
        ...
        <nav className="flex-1 py-10 px-6 space-y-2 overflow-y-auto">
          {navItems.map((item) => (
            <button
              ...
              className={`flex items-center gap-4 px-6 py-4 w-full rounded-2xl transition-all duration-300 group ${
                currentPage === item.id || (currentPage === 'admin' && item.id === 'admin-dashboard')
                  ? 'bg-dark text-white shadow-xl shadow-black/10'
                  : 'text-gray-400 hover:bg-gray-50 hover:text-dark'
              }`}
            >
              <span className={`transition-transform duration-300 ${currentPage === item.id ? 'scale-110' : 'group-hover:translate-x-1'}`}>
                {item.icon}
              </span>
